...
module.exports = function () {
  autobahnSvc.on('open', function (session) {
    ...
    session.register('<%= props.wampNamespace %>.fn.findUserSessions', findUserSessions).then(logGood, logBad);
    session.register('<%= props.wampNamespace %>.fn.findSessionUserId', findSessionUserId).then(logGood, logBad);
    session.register('<%= props.wampNamespace %>.fn.findSessionUserIds', findSessionUserIds).then(logGood, logBad);
    ...
    populateActiveSessions(session).then(function (success) {
      dumpActiveSessions();
      session.register('<%= props.wampNamespace %>.authenticate', authenticate).then(logGood, logBad);
    })
    .catch(function (err) {
